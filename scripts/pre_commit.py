...
if __name__ == "__main__":
    ...
    subprocess.run(["poetry", "--quiet", "run", "isort", "src", "--show-files"], stdout=sys.stdout, stderr=subprocess.STDOUT, cwd=app_root_directory)
    subprocess.run(["poetry", "--quiet", "run", "isort", "src"], stdout=sys.stdout, stderr=subprocess.STDOUT, cwd=app_root_directory)
    print(f"{color_heading}>> iSort done")

    # Black
    print(f"\n{color_heading}>> Running Black:")
    subprocess.run(["poetry", "--quiet", "run", "black", "src"], stdout=sys.stdout, stderr=subprocess.STDOUT, cwd=app_root_directory)
    print(f"{color_heading}>> Black done")

    # Mypy
    print(f"\n{color_heading}>> Running Mypy:")
    subprocess.run(["poetry", "--quiet", "run", "mypy", "src"], stdout=sys.stdout, stderr=subprocess.STDOUT, cwd=app_root_directory)
    print(f"{color_heading}>> Mypy done")

    # Flake8
    print(f"\n{color_heading}>> Running Flake8:")
    subprocess.run(["poetry", "--quiet", "run", "flake8", "-v", "src"], stdout=sys.stdout, stderr=subprocess.STDOUT, cwd=app_root_directory)
